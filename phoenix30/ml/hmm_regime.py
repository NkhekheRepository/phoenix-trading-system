...
def compute_hmm_features(returns: np.ndarray, vol: np.ndarray, volume_change: np.ndarray) -> dict:
    n = len(returns)
    ...
    max_train = 1000
    if n > max_train:
        idx = np.linspace(0, n - 1, max_train, dtype=int)
        obs_train = obs[idx]
    else:
        obs_train = obs
    ...
    regime_probs = hmm.predict_regime_probs_fast(obs)
    ...
    for i in range(n):
        ...
        vol_regime[i] = vol[i] / (hmm.mu[regime[i], 1] + 1e-10)
        trend_strength[i] = abs(hmm.mu[regime[i], 0]) / hmm.Sigma[regime[i], 0, 0]
